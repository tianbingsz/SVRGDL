...
DEFINE_MATRIX_TERNARY_PARAMETER_OP(SvrgUpdate, THREE_PARAMETER,
        a += - (p1 * b + p2 * c + p1 * p3 * a));
template<class T>
void BaseMatrixT<T>::svrgUpdate(BaseMatrixT& b,  // grad
                                BaseMatrixT& c,  // gradSum
                                T p1,        // learningRate,
                                T p2,        // p1 * 1/numBatches,
                                T p3) {      // decayRate
  applyTernary(ternary::SvrgUpdate<T>(p1, p2, p3), b, c);
}
